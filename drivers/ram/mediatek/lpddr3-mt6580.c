...
/*
 * MediaTek LPDDR3 driver for MT6580 SoC
*/
...
struct mtk_lpddr3_priv {
    ...
    unsigned int rkXcoarse;
    unsigned int rkXfine;

    unsigned int ucsum;
    unsigned int ucbest_step;

    unsigned int rk0_dqidly1;
    unsigned int rk0_dqidly2;
    unsigned int rk0_dqidly3;
    unsigned int rk0_dqidly4;
    unsigned int rk0_dqidly5;
    unsigned int rk0_dqidly6;
    unsigned int rk0_dqidly7;
    unsigned int rk0_dqidly8;
    unsigned int rk0_r0deldly;

    unsigned int rk1_dqidly1;
    unsigned int rk1_dqidly2;
    unsigned int rk1_dqidly3;
    unsigned int rk1_dqidly4;
    unsigned int rk1_dqidly5;
    unsigned int rk1_dqidly6;
    unsigned int rk1_dqidly7;
    unsigned int rk1_dqidly8;
    unsigned int rk1_r0deldly;

    unsigned int dqodly1;
    unsigned int dqodly2;
    unsigned int dqodly3;
    unsigned int dqodly4;
    unsigned int padctl2;
    unsigned int padctl3;
};

#ifdef CONFIG_SPL_BUILD

static unsigned int drvn = 0;
static unsigned int drvp = 0;
...
void DDR_PHY_reset_new(struct udevice *dev)
{
    DRAMC_WRITE_REG(dev, (DRAMC_READ_REG(dev, 0xf0)) | (1 << 28), 0xf0);
    DRAMC_WRITE_REG(dev, (DRAMC_READ_REG(dev, 0xf4)) | (1 << 25), 0xf4);
    udelay(1000); // delay_a_while
    DRAMC_WRITE_REG(dev, (DRAMC_READ_REG(dev, 0xf0)) & (~(1 << 28)), 0xf0);
    DRAMC_WRITE_REG(dev, (DRAMC_READ_REG(dev, 0xf4)) & (~(1 << 25)), 0xf4);
}
...
static int mtk_lpddr3_memtest(void)
{
    ...
    for (step = 1; step < 0x40000; step++)
    {
        start = readl(CFG_SYS_SDRAM_BASE);

        writel(~WALKING_PATTERN, (unsigned long)CFG_SYS_SDRAM_BASE + (step << 2));
        test = readl((unsigned long)CFG_SYS_SDRAM_BASE + (step << 2));
        if (test != ~WALKING_PATTERN || test != ~start)
        {
            printf("MEMTEST FAILED: 0x%.8x: %.8x vs %.8x\n", step<<2, test, ~WALKING_PATTERN);
            return -ENOMEM;
        }
...
void mtk_mempll_init(struct udevice *dev, int type, int pll_mode)
{
    ...
    {
        writel(0x00010020, priv->ddrphy_base + (0x0640));
    }
    else if (pll_mode== PLL_MODE_2)
    {
        writel(0x00010022, priv->ddrphy_base + (0x0640));
    }
    else
    {
    ...
    writel((0x10000000| 0x1000000 | 0x100000| 0x10000 | 0x1000| 0x100 | 0x10| 0x1), priv->ddrphy_base + (0x05cc));
    ...
    {
        writel((0x00010022  | 0x20), priv->ddrphy_base + (0x0640));
    }
    else if (pll_mode == PLL_MODE_3)
    {
        writel((0x00010020 | 0x20), priv->ddrphy_base + (0x0640));
    }
    else
    {
        writel(0x00000007, priv->ddrphy_base + (0x0640));
    ...
    if (pll_mode == PLL_MODE_2)
    {
        writel((0x00010022 | 0x20 | 0x10), priv->ddrphy_base + (0x0640));
    }
    else if (pll_mode == PLL_MODE_3)
    {
        writel((0x00010020 | 0x20 | 0x10), priv->ddrphy_base + (0x0640));
    }
    else
    {
...
static void DramcRxdatlatCal(struct udevice *dev)
{
    struct mtk_lpddr3_priv *priv = dev_get_priv(dev);
    if (priv->ucsum == 0)
        return;

    unsigned int _07c = DRAMC_READ_REG(dev, 0x7c);
    unsigned int _0e4 = DRAMC_READ_REG(dev, 0xe4);

    if (priv->ucbest_step >= 8)
    {
        _0e4 |= 0x10;
    } else {
        _0e4 &= ~0x10;
    }
    DRAMC_WRITE_REG(dev, _0e4, 0xe4);

    _07c &= ~0x70;
    _07c |= (priv->ucbest_step&0x7) << 4;
    DRAMC_WRITE_REG(dev, _07c, 0x7c);
}

static void do_dqs_gw_calib_2(struct udevice *dev)
{
    struct mtk_lpddr3_priv *priv = dev_get_priv(dev);

    DRAMC_WRITE_REG(dev, 0x10000000, 0xE0);
    DRAMC_WRITE_REG(dev, 0x100, 0x1E4);
    DRAMC_WRITE_REG(dev, priv->test2_1, 0x3C);
    DRAMC_WRITE_REG(dev, (DRAMC_READ_REG(dev, 0x40) & 0xAA000000) | 0x3FF , 0x40);

    DRAMC_WRITE_REG(dev, (DRAMC_READ_REG(dev, 0x94) & 0x80000000)
    | ((priv->rkXfine & 0x7F) << 0)
    | ((priv->rkXfine & 0x7F) << 8)
    | ((priv->rkXfine & 0x7F) << 16)
    | ((priv->rkXfine & 0x7F) << 24), 0x94);

    DRAMC_WRITE_REG(dev, (DRAMC_READ_REG(dev, 0xE0) & 0xF8FFFFFF)
    | (((priv->rkXcoarse >> 2) & 7) << 24), 0xE0);
    DRAMC_WRITE_REG(dev, (DRAMC_READ_REG(dev, 0x124) & 0xFFFFFFCC)
    | ((priv->rkXcoarse & 0x3) << 0)
    | ((priv->rkXcoarse & 0x3) << 4), 0x124);


    DDR_PHY_reset_new(dev);
}
...
static int init_lpddr3(struct udevice *dev)
{
    struct mtk_lpddr3_priv *priv = dev_get_priv(dev);
    unsigned int temp;
    unsigned int opt_rx_dqs0, opt_rx_dqs1;
    ...
    writel(0x07007047, priv->emi + 0x118);
    writel(0x2030604b, priv->emi + 0x120);
    writel(0xa0a07046, priv->emi + 0x128);
    writel(0xa0a07046, priv->emi + 0x134);

    writel(0x0d1e293a, priv->emi + 0x008);
    writel(0x09190819, priv->emi + 0x010);
    ...
    writel(0x00000000 | ((drvp<<28) + (drvn<<24) + (drvp<<12) + (drvn<<8)), priv->dramc0_base + 0x0b4);
    writel(0x00000000 | ((drvp<<28) + (drvn<<24) + (drvp<<12) + (drvn<<8)), priv->ddrphy_base + 0x0b4);
    writel(0x00000000 | ((drvp<<28) + (drvn<<24) + (drvp<<12) + (drvn<<8)), priv->dramc0_base + 0x0b8);
    writel(0x00000000 | ((drvp<<28) + (drvn<<24) + (drvp<<12) + (drvn<<8)), priv->ddrphy_base + 0x0b8);
    writel(0x00000000 | ((drvp<<28) + (drvn<<24) + (drvp<<12) + (drvn<<8)), priv->dramc0_base + 0x0bc);
    writel(0x00000000 | ((drvp<<28) + (drvn<<24) + (drvp<<12) + (drvn<<8)), priv->ddrphy_base + 0x0bc);
    ...
    writel(0x00000001, priv->dramc0_base + 0x08c);
    writel(0x00000001, priv->ddrphy_base + 0x08c);
    ...
    // (#ifndef DDR_1333) AND (#ifndef DDR_1466)
    writel(0x1b004004, priv->dramc0_base + 0x0e0);
    writel(0x1b004004, priv->ddrphy_base + 0x0e0);
    writel(0xaa080033, priv->dramc0_base + 0x124);
    writel(0xaa080033, priv->ddrphy_base + 0x124);

    writel(0x40000000, priv->dramc0_base + 0x0f0);
    writel(0x40000000, priv->ddrphy_base + 0x0f0);

    writel(priv->gddr3ctl, priv->dramc0_base + 0x0f4);
    writel(priv->gddr3ctl, priv->ddrphy_base + 0x0f4);

    temp = readl(priv->dramc0_base + 0x0f4);
    writel(temp | 0x10000000, priv->dramc0_base + 0x0f4);
    temp = readl(priv->ddrphy_base + 0x0f4);
    writel(temp | 0x10000000, priv->ddrphy_base + 0x0f4);

    temp = readl(priv->dramc0_base + 0x0f4);
    writel(temp | 0x00f00000, priv->dramc0_base + 0x0f4);
    temp = readl(priv->ddrphy_base + 0x0f4);
    writel(temp | 0x00f00000, priv->ddrphy_base + 0x0f4);
    ...
    writel(0x0110017f, priv->dramc0_base + 0x1e0);
    writel(0x0110017f, priv->ddrphy_base + 0x1e0);
    ...
    writel(0x00222222, priv->dramc0_base + 0x408);
    writel(0x00222222, priv->ddrphy_base + 0x408);
    ...
    writel(0x0b052311, priv->dramc0_base + 0x110);
    writel(0x0b052311, priv->ddrphy_base + 0x110);

    temp = readl(priv->dramc0_base + 0x01dc);
    writel(temp | 0x04000000, priv->dramc0_base + 0x01dc);
    temp = readl(priv->ddrphy_base + 0x01dc);
    writel(temp | 0x04000000, priv->ddrphy_base + 0x01dc);
    ...
    writel(priv->modereg1, priv->ddrphy_base + 0x088);
    ...
    writel(priv->modereg2, priv->dramc0_base + 0x088);
    writel(priv->modereg2, priv->ddrphy_base + 0x088);
    ...
    writel(0x00113381, priv->dramc0_base + 0x0110);
    writel(0x00113381, priv->ddrphy_base + 0x0110);

    writel((((readl(priv->dramc0_base + 0x007c)) & 0xFFFFFF8F) | ((7 & 0x07) <<4)), priv->dramc0_base + 0x007c);
    writel((((readl(priv->ddrphy_base + 0x007c)) & 0xFFFFFF8F) | ((7 & 0x07) <<4)), priv->ddrphy_base + 0x007c);
    writel((((readl(priv->dramc0_base + 0x00e4)) & 0xFFFFFFEF) | (((7 >> 3) & 0x01) << 4)), priv->dramc0_base + 0x00e4);
    writel((((readl(priv->ddrphy_base + 0x00e4)) & 0xFFFFFFEF) | (((7 >> 3) & 0x01) << 4)), priv->ddrphy_base + 0x00e4);

    writel(0xA090C08, priv->dramc0_base + 0x0210);
    writel(0xA090C08, priv->ddrphy_base + 0x0210);
    writel(0x3000300, priv->dramc0_base + 0x0214);
    writel(0x3000300, priv->ddrphy_base + 0x0214);
    writel(0xA090807, priv->dramc0_base + 0x0218);
    writel(0xA090807, priv->ddrphy_base + 0x0218);
    writel(0x4040602, priv->dramc0_base + 0x021c);
    writel(0x4040602, priv->ddrphy_base + 0x021c);
    writel(0x6090909, priv->dramc0_base + 0x0220);
    writel(0x6090909, priv->ddrphy_base + 0x0220);
    writel(0x2000100, priv->dramc0_base + 0x0224);
    writel(0x2000100, priv->ddrphy_base + 0x0224);
    writel(0x7070705, priv->dramc0_base + 0x0228);
    writel(0x7070705, priv->ddrphy_base + 0x0228);
    writel(0x1020203, priv->dramc0_base + 0x022c);
    writel(0x1020203, priv->ddrphy_base + 0x022c);
    writel(0x242B252C, priv->dramc0_base + 0x0018);
    writel(0x242B252C, priv->ddrphy_base + 0x0018);
    writel(0x242B252C, priv->dramc0_base + 0x001c);
    writel(0x242B252C, priv->ddrphy_base + 0x001c);

    writel(0x000000cc, priv->dramc0_base + 0x00cc);
    writel(0x000000cc, priv->ddrphy_base + 0x00cc);
    writel(0x0ff00ff0, priv->dramc0_base + 0x0648);
    writel(0x0ff00ff0, priv->ddrphy_base + 0x0648);
    ...
    do_dqs_gw_calib_2(dev);

    DramcRxdatlatCal(dev);

    // do_sw_rx_dq_dqs_calib
    DRAMC_WRITE_REG(dev, priv->rk0_dqidly1, 0x210);
    DRAMC_WRITE_REG(dev, priv->rk0_dqidly2, 0x214);
    DRAMC_WRITE_REG(dev, priv->rk0_dqidly3, 0x218);
    DRAMC_WRITE_REG(dev, priv->rk0_dqidly4, 0x21c);
    DRAMC_WRITE_REG(dev, priv->rk0_dqidly5, 0x220);
    DRAMC_WRITE_REG(dev, priv->rk0_dqidly6, 0x224);
    DRAMC_WRITE_REG(dev, priv->rk0_dqidly7, 0x228);
    DRAMC_WRITE_REG(dev, priv->rk0_dqidly8, 0x22c);
    DRAMC_WRITE_REG(dev, priv->rk0_r0deldly, 0x18);

    opt_rx_dqs0 = DRAMC_READ_REG(dev, 0x18);

    unsigned int bak_cona = readl(priv->emi + 0x0000);
    if ((readl(priv->emi + 0x0000) & 0x20000) != 0)
    {
        temp = readl(priv->dramc0_base + 0x110);
        writel((temp | 8), priv->dramc0_base + 0x110);
        ...
        // do_sw_rx_dq_dqs_calib
        DRAMC_WRITE_REG(dev, priv->rk1_dqidly1, 0x210);
        DRAMC_WRITE_REG(dev, priv->rk1_dqidly2, 0x214);
        DRAMC_WRITE_REG(dev, priv->rk1_dqidly3, 0x218);
        DRAMC_WRITE_REG(dev, priv->rk1_dqidly4, 0x21c);
        DRAMC_WRITE_REG(dev, priv->rk1_dqidly5, 0x220);
        DRAMC_WRITE_REG(dev, priv->rk1_dqidly6, 0x224);
        DRAMC_WRITE_REG(dev, priv->rk1_dqidly7, 0x228);
        DRAMC_WRITE_REG(dev, priv->rk1_dqidly8, 0x22c);
        DRAMC_WRITE_REG(dev, priv->rk1_r0deldly, 0x18);

        opt_rx_dqs1 = DRAMC_READ_REG(dev, 0x18);

        DRAMC_WRITE_REG(dev, (DRAMC_READ_REG(dev, 0x118) & 0xFFFFFFF8)
        | (((priv->rkXcoarse >> 2) & 7) << 0), 0x118);
        DRAMC_WRITE_REG(dev, (DRAMC_READ_REG(dev, 0x124) & 0xFFFFFF33)
        | ((priv->rkXcoarse & 0x3) << 2) | ((priv->rkXcoarse & 0x3) << 6), 0x124);

        DRAMC_WRITE_REG(dev, (DRAMC_READ_REG(dev, 0x98) & 0x80000000)
        | ((priv->rkXfine & 0x7F) << 0)
        | ((priv->rkXfine & 0x7F) << 8)
        | ((priv->rkXfine & 0x7F) << 16)
        | ((priv->rkXfine & 0x7F) << 24), 0x98);

        DRAMC_WRITE_REG(dev, opt_rx_dqs1, 0x1C);
    ...
    writel(bak_cona, priv->emi + 0x000);
    ...
    DRAMC_WRITE_REG(dev, opt_rx_dqs0, 0x1C);

    DDR_PHY_reset_new(dev);

    // do_sw_tx_dq_dqs_calib
    ...
    DRAMC_WRITE_REG(dev, priv->padctl2, 0x10);
    ...
    if ((readl(priv->emi + 0x0000) & 0x20000) != 0)
    {
        temp = readl(priv->dramc0_base + 0xe0) & 0x07000000;
        temp = ((temp >> 24) + 2) << 16;
        writel((readl(priv->dramc0_base + 0x1c4) & 0xfff0ffff) | temp, priv->dramc0_base + 0x1c4);
    }

    writel(readl(priv->dramc0_base + 0x01c0) | 0x80000000, priv->dramc0_base + 0x01c0);
    writel(readl(priv->ddrphy_base + 0x01c0) | 0x80000000, priv->ddrphy_base + 0x01c0);
    ...
    writel(readl(priv->dramc0_base + 0x00f0) | 0x08000000, priv->dramc0_base + 0x00f0);
    writel(readl(priv->ddrphy_base + 0x00f0) | 0x08000000, priv->ddrphy_base + 0x00f0);
    ...
    writel((temp | 0xfeb88200<<0), priv->ddrphy_base + 0x0640);

    temp = readl(priv->dramc0_base + 0x01dc);
    writel((temp | 0x0a000000), priv->dramc0_base + 0x01dc);
    temp = readl(priv->ddrphy_base + 0x01dc);
    writel((temp | 0x0a000000), priv->ddrphy_base + 0x01dc);
...
static int mtk_lpddr3_probe(struct udevice *dev)
{
	...
	if (priv->dramc_nao_base == FDT_ADDR_T_NONE)
		return -EINVAL;
    ...
    ofnode_read_u32(dev_ofnode(dev), "emi_cona", &priv->emi_cona);
    ofnode_read_u32(dev_ofnode(dev), "actim", &priv->actim);
    ofnode_read_u32(dev_ofnode(dev), "gddr3ctl", &priv->gddr3ctl);
    ofnode_read_u32(dev_ofnode(dev), "conf1", &priv->conf1);
    ofnode_read_u32(dev_ofnode(dev), "ddr2ctl", &priv->ddr2ctl);
    ofnode_read_u32(dev_ofnode(dev), "test2_1", &priv->test2_1);
    ofnode_read_u32(dev_ofnode(dev), "test2_3", &priv->test2_3);
    ofnode_read_u32(dev_ofnode(dev), "conf2", &priv->conf2);
    ofnode_read_u32(dev_ofnode(dev), "pd_ctrl", &priv->pd_ctrl);
    ofnode_read_u32(dev_ofnode(dev), "actim1", &priv->actim1);
    ofnode_read_u32(dev_ofnode(dev), "misctl0", &priv->misctl0);
    ofnode_read_u32(dev_ofnode(dev), "actim05t", &priv->actim05t);
    ofnode_read_u32(dev_ofnode(dev), "modereg1", &priv->modereg1);
    ofnode_read_u32(dev_ofnode(dev), "modereg2", &priv->modereg2);
    ofnode_read_u32(dev_ofnode(dev), "modereg10", &priv->modereg10);
    ofnode_read_u32(dev_ofnode(dev), "modereg63", &priv->modereg63);

    ofnode_read_u32(dev_ofnode(dev), "rkXcoarse", &priv->rkXcoarse);
    ofnode_read_u32(dev_ofnode(dev), "rkXfine", &priv->rkXfine);

    ofnode_read_u32(dev_ofnode(dev), "ucsum", &priv->ucsum);
    ofnode_read_u32(dev_ofnode(dev), "ucbest_step", &priv->ucbest_step);

    ofnode_read_u32(dev_ofnode(dev), "rk0_dqidly1", &priv->rk0_dqidly1);
    ofnode_read_u32(dev_ofnode(dev), "rk0_dqidly2", &priv->rk0_dqidly2);
    ofnode_read_u32(dev_ofnode(dev), "rk0_dqidly3", &priv->rk0_dqidly3);
    ofnode_read_u32(dev_ofnode(dev), "rk0_dqidly4", &priv->rk0_dqidly4);
    ofnode_read_u32(dev_ofnode(dev), "rk0_dqidly5", &priv->rk0_dqidly5);
    ofnode_read_u32(dev_ofnode(dev), "rk0_dqidly6", &priv->rk0_dqidly6);
    ofnode_read_u32(dev_ofnode(dev), "rk0_dqidly7", &priv->rk0_dqidly7);
    ofnode_read_u32(dev_ofnode(dev), "rk0_dqidly8", &priv->rk0_dqidly8);
    ofnode_read_u32(dev_ofnode(dev), "rk0_r0deldly", &priv->rk0_r0deldly);

    ofnode_read_u32(dev_ofnode(dev), "rk1_dqidly1", &priv->rk1_dqidly1);
    ofnode_read_u32(dev_ofnode(dev), "rk1_dqidly2", &priv->rk1_dqidly2);
    ofnode_read_u32(dev_ofnode(dev), "rk1_dqidly3", &priv->rk1_dqidly3);
    ofnode_read_u32(dev_ofnode(dev), "rk1_dqidly4", &priv->rk1_dqidly4);
    ofnode_read_u32(dev_ofnode(dev), "rk1_dqidly5", &priv->rk1_dqidly5);
    ofnode_read_u32(dev_ofnode(dev), "rk1_dqidly6", &priv->rk1_dqidly6);
    ofnode_read_u32(dev_ofnode(dev), "rk1_dqidly7", &priv->rk1_dqidly7);
    ofnode_read_u32(dev_ofnode(dev), "rk1_dqidly8", &priv->rk1_dqidly8);
    ofnode_read_u32(dev_ofnode(dev), "rk1_r0deldly", &priv->rk1_r0deldly);


    ofnode_read_u32(dev_ofnode(dev), "dqodly1", &priv->dqodly1);
    ofnode_read_u32(dev_ofnode(dev), "dqodly2", &priv->dqodly2);
    ofnode_read_u32(dev_ofnode(dev), "dqodly3", &priv->dqodly3);
    ofnode_read_u32(dev_ofnode(dev), "dqodly4", &priv->dqodly4);
    ofnode_read_u32(dev_ofnode(dev), "padctl2", &priv->padctl2);
    ofnode_read_u32(dev_ofnode(dev), "padctl3", &priv->padctl3);


    mtk_mempll_init(dev, DDR1066, PLL_MODE_1);
...
static const struct udevice_id mtk_ddr3_ids[] = {
	{ .compatible = "mediatek,mt6580-dramc" },
	{ }
};
